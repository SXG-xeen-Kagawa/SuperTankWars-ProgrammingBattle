...
- connpassから参加した方は、イベントにエントリーし、受付番号（または参加ID）が発行されていることを確認してください。
（Peatixから来た方はエントリー不要です。受付番号は作成時に自動生成されています）
- アイコン画像サイズは最大256×256です。
- 公序良俗に反する画像や名前は設定しないでください。
  - 運営チームの審査で不可となった場合は、勝手ながら名前や画像を変更させていただきますのでご了承ください。
